...
class TestInvitationEmail:
    ...
    def test_send_invitation_email_success(
        self, workspace, owner_user, settings
    ) -> None:
        """Test that invitation email is sent successfully."""
        from core.views.members import send_invitation_email
        ...
        result = send_invitation_email(
            invitation, "https://example.com/accept/test-token/"
        )

        assert result is True
        assert len(mail.outbox) == 1
        assert mail.outbox[0].to == ["newuser@test.com"]
        assert workspace.name in mail.outbox[0].subject
        assert "https://example.com/accept/test-token/" in mail.outbox[0].body

    def test_send_invitation_email_includes_inviter_name(
        self, workspace, owner_user, settings
    ) -> None:
        """Test that email includes inviter's name."""
        from core.views.members import send_invitation_email
        from django.core import mail

        settings.EMAIL_BACKEND = "django.core.mail.backends.locmem.EmailBackend"

        owner_user.first_name = "John"
        owner_user.last_name = "Doe"
        owner_user.save()
        ...
        send_invitation_email(invitation, "https://example.com/accept/")

        assert "John Doe" in mail.outbox[0].body

    def test_send_invitation_email_falls_back_to_email(
        self, workspace, owner_user, settings
    ) -> None:
        """Test that email falls back to inviter's email if no name."""
        from core.views.members import send_invitation_email
        from django.core import mail
        ...
        assert owner_user.email in mail.outbox[0].body

    def test_invite_member_sends_email(
        self, authenticated_owner_client, workspace, settings
    ) -> None:
        """Test that inviting a member sends an email."""
        from django.core import mail
        ...
        assert response.status_code == 302
        assert len(mail.outbox) == 1
        assert mail.outbox[0].to == ["invited@test.com"]
