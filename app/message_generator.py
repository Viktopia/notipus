...
class MessageGenerator:
    ...
    UPGRADE_TEMPLATES = [
        "🎉 🚀 Awesome upgrade! {customer_name} is growing fast, leveling up from {old_plan} to {new_plan}!",
        "⭐️ 🌟 {customer_name} just supercharged to {new_plan}! They're scaling up!",
        "🚀 💪 Power up! {customer_name}'s expanding to {new_plan} and we're here for it!",
        "🎉 ⭐️ Next level! {customer_name}'s moving up to {new_plan}! Things are getting serious!",
        "🚀 🌟 Leveled up! {customer_name}'s growing with {new_plan} powers!"
    ]
    ...
    def _ensure_required_fields(self, event: Dict[str, Any], required_fields: List[str]) -> Dict[str, Any]:
    ...
    def trial_ending(self, event: Dict[str, Any]) -> str:
        ...
        event = self._ensure_required_fields(event, ['customer_name', 'popular_features'])
        event['popular_features'] = self._format_features(event.get('popular_features', []))
    ...
    def plan_upgrade(self, event: Dict[str, Any]) -> str:
        ...
        event = self._ensure_required_fields(event, ['customer_name', 'old_plan', 'new_plan'])
